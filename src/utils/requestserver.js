...
instance.interceptors.response.use(
  ...
  async function (error) {
    ...
    if (error.response && error.response.status === 401) {
      try {
        ...
        const refreshResponse = await axios.post(
          "http://localhost:8088/api/v1/auth/refresh-token",
          {
            refreshToken,
          }
        );
    ...
    if (error.response && [301, 302].includes(error.response.status)) {
      console.warn(
        "Redirect detected. Check server authentication or endpoint configuration."
      );
      return Promise.reject(
        new Error(
          "Server redirected the request. Please check authentication or server configuration."
        )
      );
    }
...
export const edit = (path, data, config = {}) =>
